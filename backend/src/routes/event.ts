...
router.post("/", authMiddleware, checkRole(["ORGANIZER"]), async (req: Request, res: Response) => {
  try {
    const { name, description, price, availableSeats, startDate, endDate, bankAccountNumber } = req.body;
    const user = req.user as any;

    if (!name || !price || !availableSeats || !startDate || !endDate || !bankAccountNumber) {
...
router.patch("/:id", authMiddleware, checkRole(["ORGANIZER"]), async (req: Request, res: Response) => {
  try {
    ...
    if (!event) {
      return res.status(404).json({ message: "Event tidak ditemukan atau bukan milik Anda" });
    }
